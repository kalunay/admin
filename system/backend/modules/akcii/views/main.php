...
<form action="akcii/copydellgroup" method="post" onsubmit="return confirm('<?=$this->lang->line('action_confirm')?>');">
...
<td class="t-center"><? if (isset($i['image']) and $i['image']):?>
 <a href='<?=$image_path.$i['image']?>' target=_blank><img src="<?=$image_path.$i['image']?>" height=30px></a><?endif;?></td>
 <? } ?>
<td class="t-center">
<?=anchor("akcii/edit/".$i['id'],$this->lang->line('edit'),array('class'=>'ico-edit'))?> 
<?=anchor("akcii/delete/".$i['id'],
...
<? endforeach; ?>
  ...
  <p><?=anchor("akcii/add/","<span>".$this->lang->line('add')."</span>",array('class'=>'btn-create'))?></p>
