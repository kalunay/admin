...
class Model_akcii extends Model {
 function Model_akcii(){
  parent::Model();
 ...
 function get_info($id){
  ...
  return reset($q->result_array());
	...
	$this->db->where('table_name','akcii');
...
"pos"=>$aData['pos'],
  ...
  $this->db->insert('akcii',$aInsert);
  
  		//////////////////////////////////////// lang
	$this->db->order_by('id','desc');
	$last = $this->db->get('akcii');
	if($last->num_rows()){
...
"pos"=>$aData['pos'],
 ...
 function copyDellGroup($type,$id){
  switch ($type){
   case "del":
   foreach ($id as $item){
    $this->db->query("DELETE FROM `akcii` WHERE `id` = $item");
   }
   break;
